...
class FunctionSignature {
	...
	public LinkedHashMap<String,Integer> varNumMap = new LinkedHashMap<String,Integer>();
...
public class TypeBuilderVisitor<R> implements GJNoArguVisitor<R> {
	...
	public LinkedHashMap<String, LinkedHashMap<String, FunctionSignature>> classTofunctionMap 
			= new LinkedHashMap<String, LinkedHashMap<String, FunctionSignature>>();

	public LinkedHashMap<String,LinkedHashMap<String,String>> classToVarMap
   ...
   private void printClassInfo() {
      Set<String> classes = classTofunctionMap.keySet();
      Iterator<String> _class = classes.iterator();
      
      System.out.println("class variables : \t" + classToVarMap.toString());
      //System.out.println("function map : \t" + functionMap.toString());
      while( _class.hasNext() ) {
    	  String classname = _class.next();
	      Collection<FunctionSignature> signatures = classTofunctionMap.get(classname).values();
...
private void inject(LinkedList<String> list, String _class) {
	   ...
	   classToVarMap.get(_class).putAll(classToVarMap.get(baseClass));
	   LinkedHashMap<String, FunctionSignature> concreteMethods = classTofunctionMap.get(_class);
	   LinkedHashMap<String, FunctionSignature> baseMethods = classTofunctionMap.get(baseClass);
...
/**
   ...
   public R visit(ClassDeclaration n) {
	  ...
	  if(!classToVarMap.containsKey(className))
		  classToVarMap.put(className, vardecl);
      n.f3.accept(this);
      n.f4.accept(this);
      n.f5.accept(this);
      
      classTofunctionMap.put(className, signatureMap);
   ...
   public R visit(VarDeclaration n) {
      ...
      if(infunc)
    	  ...
    	  if(classToVarMap.containsKey(className))
    		  classToVarMap.get(className).put(idtfr, type);
    	  else{
